...
Based on the result of the exploratory analysis, a prectitive model is built which strikes the balance of accuracy, kappa (a measure of concordance) and computational performance in an computational environment with limited computational resources.  The prection model uses the random forest method with a value of the tuning parameter *mtry* equal to 8.  The number of features used to build the model is 53.  The model accuracy and kappa are 0.999 and 0.998, respectively.
...
## Predictive Modeling ##
The exploratory analysis only touches few aspects of the data.  The characteristics of the data are not fully explored.  Without fully understanding the characteristics of the data, it is advantageous to employ non-parametric approaches to build a predictive model.

The Random Forest method is performed and the result of the modeling is given in the following
```{r}
set.seed(340091)
...
The computation time to build the model is about 12 minutes on my laptop computer with Intel CORE i5 running Windows 10.
...
The prediction model has accuracy and kappa with the value of 0.999 and 0.998, respectively.  The cross-validation accuracy is 0.999.  In other words, the cross-validation error rate is 0.001, which is an estimate of out-of-sample error rate.
